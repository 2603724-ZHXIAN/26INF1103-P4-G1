    ...
    while True:
        ...
        while choice not in {"1", "2", "3", "4", "5", "6"}:
            print(
                ...
                f"between 1 and 6.{RESET}"
            )
            choice = input("Select an option (1-6): > ").strip()
        print(f"{BLUE}[I/O Manager]{RESET} 🛠️  Option [{choice}] selected.")

        # --- Option 1: text/email body ---
        if choice == "1":
            print("\n📧 -- Analyze Suspicious Text / Scam Email Message --")
            print("Paste the message below. Type END on a new line to finish.\n")
            lines = []
            while True:
                line = input("> ")
            ...
            print(
                f"{BLUE}[I/O Manager]{RESET} 🛠️  Captured text input "
                f"({len(text)} characters)."
            )
            print(
                f"{BLUE}[I/O Manager]{RESET} 🛠️  Submission of text is "
                f"being processed, please wait..."
            )
            ...
            while True:
                ...
                if validators.url(url) or validators.domain(url):
                    ...
                    print(
                        ...
                        f"URL/domain (e.g. example.com or https://example.com"
                        f"/path). Try again.{RESET}"
                    )
                    continue

            print(
                f"{BLUE}[I/O Manager]{RESET} 🛠️  Submission of url "
                f"website is being analysed..."
            )
            ...
            while True:
                ...
                if not os.path.isfile(path):
                    ...
                    if retry == "y":
                        ...
                        print(
                            f"{BLUE}[I/O Manager]{RESET} 🛠️  User proceeded "
                            f"with an unverified file path."
                        )
                        break
                ...
                #     print(f"  {YELLOW}⚠️  '{ext}' is an invalid extension.{RESET}")
                #     proceed = input("    Analyze anyway? (y/n): > ").strip().lower()
                #     if proceed != "y":
                #         continue

                print(f"\n{GREEN}✅ File '{filename}' accepted.{RESET}\n")
                break

            print(
                f"{BLUE}[I/O Manager]{RESET} 🛠️  Submission of file "
                f"'{filename}' is being handed off for analysis..."
            )
        ...
        # --- Option 4: Historical database query ---
            ...
            while sub_choice not in {"1", "2", "3"}:
                print(
                    f"  {YELLOW}⚠️  Invalid choice. Please enter a "
                    f"number 1, 2 or 3.{RESET}"
                )
                sub_choice = input("Select an option (1-3): > ").strip()
            ...
            if sub_choice == "1":
                ...
                print(
                    f"{BLUE}[I/O Manager]{RESET} 🛠️  Querying records for "
                    f"keyword '{keyword}'."
                )
                ...
                while lvl_choice not in levels:
                    ...
                    lvl_choice = input("Select risk level (1-3): > ").strip()
                print(
                    f"{BLUE}[I/O Manager]{RESET} 🛠️  Querying records at "
                    f"risk level '{levels[lvl_choice]}'."
                )
        ...
        # --- Option 5: Common Threat Summaries (Low/Moderate/High) ---
            ...
            print(f"{BLUE}[I/O Manager]{RESET} 🛠️  Fetching trending attacks.")
            # TODO: call data_manager.get_trending_threats() here
        ...
        # --- Option 6: Exit ---
            ...
            if confirm == "y":
                print(
                    f"{BLUE}[I/O Manager]{RESET} 🛠️  Session terminated by "
                    f"user. Goodbye ~"
                )
                sys.exit(0)
        print()
